...
df_num <- df[,c("pricem2","bedrooms","bathrooms","floors", "waterfront","view","condition","age")]
...
reg <- lm(df_num$pricem2~df_num$age)
...
plot(df_num$pricem2,axes=TRUE,rstudent(reg),xlim=c(0,8650),ylim=c(0,6),xlab="Prix m2",ylab="Résidus studentisés",main="Graphique des résidus studentisés : Prix m2 x Age")
bptest(reg) # Breusch-Pagan Test
# homoscédasticité : rejeté
shapiro.test(res) # p-value<0.05 donc normalité des résidus vérifiée

ggplot(df_num,aes(y=pricem2,x=age))+
  geom_point(col="#0E1A44")+
  geom_smooth(formula = y ~ x,method="lm",se=TRUE,col='red')+
  labs(title = paste("Prix m2 x Age : prix = ", round(coefficients(reg)[1],2), " + age * ", round(coefficients(reg)[2],2), "+/- [",confidence1,";",confidence2,"] (95%) \n"))+
  xlim(min(df_num$age), 114)+ # 4 outliers non représentés
  ylim(min(df_num$pricem2), 11000) # 3 outliers non représentés

## pricem2 x bedrooms ##

reg <- lm(df_num$pricem2~df_num$bedrooms)
...
acf(res) # indépendance des résidus 
mean(res) # nullité de l'ésperance des résidus 
plot(df_num$pricem2,axes=TRUE,rstudent(reg),xlim=c(0,11000),ylim=c(0,8),xlab="Prix m2",ylab="Résidus studentisés",main="Graphique des résidus studentisés : Prix x Nombre de salles de bains")
bptest(reg) # Breusch-Pagan Test
# homoscédasticité : acceptée
shapiro.test(res) # p-value<0.05 donc normalité des résidus vérifiée

ggplot(df_num,aes(y=pricem2,x=bedrooms))+
  geom_point(col="#4D305B")+
  geom_smooth(formula = y ~ x,method="lm",se=TRUE,col='red')+
  labs(title = paste("Prix m2 x Nombre de salles de bains : prix = ", round(coefficients(reg)[1],2), " + bedrooms * ", round(coefficients(reg)[2],2), "+/- [",confidence1,";",confidence2,"]\n"))+
  xlim(min(df_num$bedrooms), max(df_num$bedrooms))+ # 4 outliers non représentés
  ylim(min(df_num$pricem2), 15000) # 3 outliers non représentés
...
reg_multiple = lm(pricem2 ~  bedrooms + bathrooms + floors + waterfront +view + condition + age,data = df_num)
...
# modèle vide
model <- lm(pricem2 ~ 1, data = df_num)

# variables explicatives
var_list <- c("waterfront", "bedrooms", "bathrooms", "floors","view","condition","age") 

while (length(var_list) > 0) { # tant que var_list est non vide
  AIC_vec <- c() # vecteur de critères AIC
  for (i in 1:length(var_list)) {
    # formule : pricem2 ~ i eme var
    formule <- as.formula(paste("pricem2 ~", paste(c(attr(terms(model), "term"), var_list[i]), collapse = "+")))
  ...
  if (AIC_vec[min_AIC_indice] < AIC(model)) { # cette AIC minimum est-il inferieur au AIC de notre modèle actuel ? 
    ...
    model <- update(model, formula = as.formula(paste("pricem2 ~", paste(c(attr(terms(model), "term"), var_list[min_AIC_indice]), collapse = "+"))))
...
while (length(var_list) > 0) { # tant que var_list est non vide
  ...
  if (BIC_vec[min_BIC_indice] < BIC(model)) { # cette BIC minimum est-il inferieur au BIC de notre modèle actuel ? 
    # si oui: on remplace notre ancien modèle par le nouveau modèle qui est meilleur
    model <- update(model, formula = as.formula(paste("pricem2 ~", paste(c(attr(terms(model), "term"), var_list[min_BIC_indice]), collapse = "+"))))
...
coef(model) # Avec le critère BIC, on perd les variables floors et conditions
model # modèle final
summary(model)
...
df_invent$prix_m2_predit <- predictions
...
# Choisir une variable à représenter parmi : pricem2,price,bedrooms,bathrooms,floors,waterfront,view,condition,yr_built,age,m2_living
var="price"
ggplot() +
  ...
  scale_fill_viridis(name="Prix moyen",option = "plasma",direction = -1) + # ou direction = -1
  #scale_fill_gradient2(name="Prix m2",low = "green", mid = "#F7F9A8", high = "brown", midpoint = mean(df$pricem2))+
  # legende
  labs(
    title = "Moyenne du prix des propriétés vendues",
