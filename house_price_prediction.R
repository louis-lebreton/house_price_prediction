...
library(tidyr)
library(dplyr)
library(ggcorrplot) # pour réaliser une matrice de corrélation
library(openxlsx) # pour enregister des fichiers excel
library(lmtest) # pour effectuer le bptest
library(geojsonio) # pour l'ouverture des fichiers geojson
library(broom) # pour utiliser la fonction tidy
library(viridis) # pour obtenir des palettes de couleurs
library(texreg)
...
# conversion des variables square feet en m2
...
# ajout de la varialbe prix au m2
...
# exportation du data frame en fichier Excel
...
boxplot(df_num$price,main="price",col="#09aec8")
...
# création de données en format long
...
shapiro.test(res) # p-value<0.05 donc normalité des résidus non vérifiée
...
ggplot(df_num,aes(y=price,x=m2_living))+
  ...
  xlim(min(df_num$m2_living), 700)+ 
  ylim(min(df_num$price), 2000000) # 45 valeurs extremes non représentées

htmlreg(reg, file = "reg_price_m2living.html") # exportation des résultats sous format html
...
htmlreg(reg, file = "reg_price_bathrooms.html") # exportation des résultats sous format html
...
# Test avec la variable bathrooms
reg_multiple = lm(price ~  m2_living + bathrooms + bedrooms + floors + waterfront +view + condition + age,data = df_num)
summary(reg_multiple)
...
htmlreg(model, file = "modeli_reg_multiple.html") # exportation des résultats sous format html

# data frame inventé de 3 maisons : modeste, moyenne, luxueuse
df_invent <- data.frame(name=c("Maison modeste","Maison moyenne","Maison luxueuse"),
                        waterfront = c(0, 0, 1),
                        bedrooms = c(1, 3, 7),
                        bathrooms = c(1, 2, 5),
                        floors = c(1, 2, 3),
...
# Choisir une variable à représenter parmi :
# price, bedrooms, bathrooms, floors, waterfront, view, condition, yr_built, age, m2_living
var="price"
ggplot() +
